...
- **Product extraction** – Reads the product name, image, current price, and selected variant/type (e.g. Color: Red / Size: L) from the page using DOM parsing
...
- Variant/type detection (Color, Size, etc.) reads whichever option is currently marked "selected" on the page; if a site's layout doesn't match the selectors in `config/selectors.json`, the variant badge may not appear for that product
